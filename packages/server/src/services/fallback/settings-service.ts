...
function getConfigDir(): string {
	return join(homedir(), '.config', 'quotio');
}
...
export async function saveFallbackConfiguration(config: FallbackConfiguration): Promise<void> {
	const configPath = getConfigPath();
...
export async function getVirtualModel(id: string): Promise<VirtualModel | null> {
	const config = await loadFallbackConfiguration();
	return config.virtualModels.find((vm) => vm.id === id) ?? null;
}

export async function addVirtualModel(name: string): Promise<VirtualModel | null> {
	const config = await loadFallbackConfiguration();
...
export async function updateVirtualModel(model: VirtualModel): Promise<boolean> {
	const config = await loadFallbackConfiguration();
...
export async function renameVirtualModel(modelId: string, newName: string): Promise<boolean> {
	const config = await loadFallbackConfiguration();
	const trimmedName = newName.trim();

	const isDuplicate = config.virtualModels.some(
		(vm) => vm.id !== modelId && vm.name.toLowerCase() === trimmedName.toLowerCase(),
	);
...
export async function removeFallbackEntry(modelId: string, entryId: string): Promise<boolean> {
	const config = await loadFallbackConfiguration();
	const model = config.virtualModels.find((vm) => vm.id === modelId);
...
export async function moveFallbackEntry(
	...
	const sorted = [...model.fallbackEntries].sort((a, b) => a.priority - b.priority);
